...
## 진행 상태 (2026-07-03 3차 갱신 — Phase 3 풀가동 수렴 = 전 Phase 완료)

- **Phase 3 완료 (LOCAL)**: 20문서 플릿(seed 42, KEEP_WORD 생성기 = 문서당 진짜 단절 ~5건) —
  **CONVERGED (clean x2, rounds=2)**. 라운드당 문서별 unique_fixed 3~9 어절(R-D 실작동),
  A3 코드리뷰·A4 비전 전 문서 전 라운드 PASS, 무결성 3종 PASS.
- **Phase 3 중 A5 자기수정 4건** (엔진/도구 자동 범위, SKILL.md 무변경):
  1. **진동 근본 해결 — 안정 어절 키**: 상태 키를 단절-텍스트에서 (문단 프리픽스, 어절 시작 오프셋)로.
     위치 드리프트 시 새 어절 오인 -> 약한 자간이 강한 자간을 덮어쓰는 회귀 -> 진동이던 것을
     사다리 단조 상승으로 전환(doc03 11 iter 진동 -> 6 iter 수렴).
  2. **반대 방향 폴백**: 자연 방향 사다리 소진 시 반대 방향(narrow<->widen) 사다리 시도(규칙2 일반화),
     전환 시 기존 자간 절대값 0 원복 후 시작.
  3. **육안 예외 사다리(-24/-28/-32/-36)**: 기하상 +-20 불가 어절(예: 27자 어절 + 긴 선행 텍스트 =
     필요 축소 26~30%+) — 스펙 "하한 -20% 육안 예외 허용" 조항 구현. doc03=-24, doc15=-36 에서 해소.
  4. **매핑 창 확장(14->28자 우선)**: 동일 긴 어절 중복 등장 시 단절점이 어절 내부 깊숙이면 작은 창이
     모호(2곳 매칭)->unmappable 이던 것을 어절 밖 문맥으로 유일 매핑(doc15/20 잔존 해소).
- **추가 하네스 결함 1건**: run_harness subprocess text=True(cp949)가 UTF-8 한글 로그에서
  UnicodeDecodeError 크래시/행 -> encoding='utf-8', errors='replace' 명시(E59 계열, 원장 등재).
- **수용기준 대비**: (B) 렌더-인-루프 엔진 5요소 충족·실물 107어절 실증, (C) 20문서 생성->독립수정->
  코드리뷰+비전 이중검증->자기수정->클린 2연속 달성. (A) SKILL.md 는 기존 R-D 절이 정확해 무변경
  (하네스 실증으로 유효성 확인). 남은 육안 확인 = 예외 자간(-24~-36) 적용부 시각 품질(사용자 열람).

---

## 진행 상태 (2026-07-03 2차 갱신 — Phase 2 파일럿 수렴)

- **Phase 2 완료 (LOCAL)**: messy 5문서(seed 11) 5-에이전트 파이프라인 — **CONVERGED (clean x2, rounds=2)**.
  A2(microalign AB + linefix R-D)·A3(codereview 기대치 대조)·A4(비전 단절 실측) 전 문서 전 라운드 PASS.
- **파일럿 1차가 적발한 결함 + A5 자기수정 2건**:
  1. A4 간헐 `n/a`(같은 문서가 라운드별 통과/실패 요동) — 원인 = **A2 렌더 직후 무간격 A4 렌더의
     COM Open 경합**(E58 보강 항목과 동일). 수정 = `render_pdf` 에 **2s 백오프 + fresh Dispatch 1회
     재시도** 내장(계획 R1 구현). 재가동에서 20 연속 렌더 무사고.
  2. A4 실패 시 원인 비표시(관측성 결함) — run_harness 가 `[A4 error]` 본문 로그 출력하도록 보강.
- **정직 한계(기존 백로그 (d) 재확인)**: 합성문서는 렌더 시 진짜 어절 단절이 생기지 않아(페이지 셋업 부재)
  파일럿의 R-D 경로는 iters=0 통과(vacuous). R-D 실증은 Phase 1 실물 문서(107 어절)가 담당.
  Phase 3 전에 생성기 페이지 지오메트리(실물 sectionPr 이식) 보강으로 합성 플릿에서도 R-D 를 발화시킬 것.
- **다음 = Phase 3 풀가동**: 생성기 페이지 셋업 보강 -> 20문서 loop-until-dry(+실물 문서 1건 동승 권장).
